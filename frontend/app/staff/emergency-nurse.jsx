...
export default function EmergencyNurseScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (!isUpdatingId) loadData(false, true);
    }, AUTO_REFRESH_MS);

    return () => clearInterval(timer);
  }, [isAuthLoading, isUpdatingId]);
            ...
            {isLoading ? (
              ...
              incidents.map((incident) => (
                ...
                >
                  ...
                  {incident.mediaAttachments?.[0]?.filePath ? (
                    <Image
                      source={{
...
const styles = StyleSheet.create({
  ...
  badgeRow: { flexDirection: "row", flexWrap: "wrap", marginBottom: 8, gap: 8 },
